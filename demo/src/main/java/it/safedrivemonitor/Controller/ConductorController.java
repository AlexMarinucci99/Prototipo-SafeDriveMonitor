...
public class ConductorController {
    ...
    private void loginOrCreateDriver(String name, String id) {
        // Creiamo la tabella drivers se non esiste
        try (Connection conn = monitoringController
                .getDbManager()
                .getConnection()) {
            String sqlTable = """
            ...
            // Controlla se esiste
            String sqlCheck = "SELECT driver_id FROM drivers WHERE driver_id=? AND driver_name=?";
            try (PreparedStatement pstmt = conn.prepareStatement(sqlCheck)) {
                ...
                if (!rs.next()) {
                    ...
                    try (PreparedStatement pstmt2 = conn.prepareStatement(sqlInsert)) {
                        ...
                        System.out.println("Nuovo driver creato: " + name + " (" + id + ")");
                    }
                } else {
                    System.out.println("Driver già esistente: " + name + " (" + id + ")");
                }
            }
